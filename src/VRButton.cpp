...
VRButton::VRButton(std::string name, std::string text, bool isContinous) : VRMenuElement(name, text), m_isContinous(isContinous), m_clicked(false)
{
...
void VRButton::updateIteration()
{
	if (m_isContinous && m_clicked)
	{
		m_menu->sendEvent(this);
	}
}
